...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!sessionRef.current) { setError('Session expired — please request a new reset link.'); return; }
    setError('');
    setLoading(true);

    const supabase = supabaseRef.current;
    // Re-establish the session explicitly before updating — the client can lose
    // it between the auth event and the submit if the internal state was cleared.
