...
export function firstDef(definitions) {
  ...
  // Prefer first segment that isn't secondary/classical and cleans to a non-empty string.
  // Skips CL:-only segments (clean to ""). Falls back to any non-empty cleaned segment.
  const preferred =
    segments.find(d => !SECONDARY_ANNOT_RE.test(d.trim()) && cleanOneDef(d).trim().length > 0)
