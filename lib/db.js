...
export async function getDueCards(userId, hskLevel) {
  const now = new Date().toISOString();

  // Fetch all entries + all progress in parallel
  ...
  for (const entry of allEntries) {
    ...
    if (!p) {
      ...
      notYetDue.push({ ...entry, ease_factor: p.ease_factor, interval: p.interval, reviews: p.reviews });
    }
  }

  return [...fisherYates(due), ...fisherYates(newCards), ...fisherYates(notYetDue)];
}
