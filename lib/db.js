...
function mergePinyinGroup(group) {
  ...
  const presorted = group.slice().sort((a, b) => {
    ...
    const aBase = (a.pinyin || '').replace(/\d$/, '');
    const bBase = (b.pinyin || '').replace(/\d$/, '');
    if (aBase === bBase) {
  ...
  const best = pyMerged[0];
  const uniquePinyins = pyMerged.map(pg => pg.pinyin).filter(Boolean);
  if (pyMerged.length === 1) return best;
...
export async function getWordOfDay() {
  ...
  // Fetch all pages in parallel: countQuery must be a complete count query (with .select);
  // dataQFn(from, to) returns a range query. Reduces wall time from ~10s to ~1s.
  async function fetchPoolParallel(countQuery, dataQFn) {
    const { count } = await countQuery;
    if (!count) return [];
    const pages = await Promise.all(
      Array.from({ length: Math.ceil(count / 1000) }, (_, i) => dataQFn(i * 1000, i * 1000 + 999))
    );
    return pages.flatMap(p => p.data || []);
  }

  // Prime stride: each day jumps ~9001 positions in the pool, so consecutive days land on
  // completely different characters rather than alphabetical neighbours.
  const POOL_STRIDE = 9001;

  if (isChengyu) {
    const all = await fetchPoolParallel(
      supabase.from('entries').select('*', { count: 'exact', head: true }).eq('is_chengyu', true),
      (from, to) => supabase.from('entries')
    ...
    const selected = unique[(dayOfYear * POOL_STRIDE) % unique.length];
    const { data: rows } = await supabase.from('entries')
      .select('simplified, traditional, pinyin, definitions, hsk_level, frequency_rank')
  ...
  // Regular words: only HSK-tagged entries, ordered by learner relevance.
  const all = await fetchPoolParallel(
    supabase.from('entries').select('*', { count: 'exact', head: true })
      ...
      .not('definitions', 'ilike', '(archaic)%'),
    (from, to) => supabase.from('entries')
      .select('simplified, hsk_level, frequency_rank')
      .not('is_chengyu', 'is', true)
      .not('hsk_level', 'is', null)
      .not('definitions', 'ilike', 'variant of%')
      .not('definitions', 'ilike', 'old variant of%')
  ...
  // Deduplicate: first occurrence per simplified = lowest hsk_level + best frequency_rank
  const seen = new Set();
  const unique = all.filter(e => seen.has(e.simplified) ? false : (seen.add(e.simplified), true));
  if (!unique.length) return null;

  const selectedSimplified = unique[(dayOfYear * POOL_STRIDE) % unique.length].simplified;
  const { data: allRows } = await supabase.from('entries')
    .select('simplified, traditional, pinyin, definitions, hsk_level, frequency_rank')
    .eq('simplified', selectedSimplified)
    .not('definitions', 'ilike', '(classical)%')
    .not('definitions', 'ilike', '(archaic)%');
