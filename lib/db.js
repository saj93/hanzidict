...
async function searchChinese(q, raw = false) {
  ...
  const exact = [...bySimplified.values()].map(group => {
    // Pick best: lowest HSK → lowest frequency_rank → most definitions → alphabetical pinyin
    // This matches sortByHskPinyin in processExactMatches so dropdown and word page agree
