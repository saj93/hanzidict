...
export async function getWordOfDay() {
  ...
  // Build a deduplicated pool of unique simplified characters (one per character,
  // not one per DB row) so the same character is never selected on two different days.
  // We fetch only the sort-key fields and deduplicate in JS; this runs once per day.
  if (isChengyu) {
    let all = [];
    let from = 0;
    while (true) {
    ...
    const selected = unique[dayOfYear % unique.length];
    const { data: rows } = await supabase.from('entries')
      .select('simplified, traditional, pinyin, definitions, hsk_level, frequency_rank')
  ...
  const selectedSimplified = unique[dayOfYear % unique.length].simplified;
