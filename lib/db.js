...
async function searchChinese(q) {
  ...
  // Prefer non-variant exact matches; fall back to variant ones only if nothing else
  const exactNonVariant = exactRaw.filter(e => !isVariantEntry(e.definitions));
  const exact = exactNonVariant.length > 0 ? exactNonVariant : exactRaw;
