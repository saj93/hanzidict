...
export async function getHskPage(hskLevel, page = 1, limit = 50) {
  ...
  // Priority: lowest HSK → alphabetically earliest pinyin (lower tone = more basic reading)
  // → most definitions for same-pinyin entries.
  // frequency_rank is NOT used: it's stored per-simplified, not per-reading, so different
  // readings of the same character can have wildly different values (e.g. 干 gàn=569, gān=1029).
  const bySimplified = new Map();
  for (const entry of all) {
    ...
    if (!existing) {
      ...
      const pyComp = (entry.pinyin || '').localeCompare(existing.pinyin || '');
      if (pyComp < 0) { bySimplified.set(key, entry); continue; }
      if (pyComp > 0) continue;
...
export async function getWordOfDay() {
  ...
  if (isChengyu) {
    ...
    // Only HSK-tagged entries — excludes classical/literary vocabulary with no HSK level.
    // Order HSK 1 first, then by frequency within each level for learner-relevance.
    countQ = countQ
      .not('is_chengyu', 'is', true)
      .not('hsk_level', 'is', null)
      .not('definitions', 'ilike', 'variant of%')
  ...
  // For chengyu (usually single entries) return directly; for regular words,
  // fetch all rows for the selected simplified and merge like processExactMatches
  // so the WoD card shows the same primary definition as the word page.
  if (isChengyu) return { ...data[0], isChengyu };

  const selectedSimplified = data[0].simplified;
  ...
  return { ...(merged ?? data[0]), isChengyu: false };
}
