...
export async function getWordOfDay() {
  ...
  if (overrideRow?.value) {
    ...
    if (merged) return { ...merged, isChengyu: false, isOverride: true };
  }
  ...
  if (isChengyu) {
    // Chengyu: ordered by priority then frequency, simple modulo selection
    const { count: chengyuCount } = await supabase.from('entries')
      .select('*', { count: 'exact', head: true })
      .eq('is_chengyu', true);
    console.log(`[getWordOfDay] isChengyu=true dayOfYear=${dayOfYear} totalChengyu=${chengyuCount}`);
    if (!chengyuCount) return null;

    const chengyuPages = await Promise.all(
      Array.from({ length: Math.ceil(chengyuCount / 1000) }, (_, i) =>
        supabase.from('entries')
          .select('simplified, chengyu_priority, frequency_rank')
          .eq('is_chengyu', true)
    ...
    const allChengyu = chengyuPages.flatMap(p => p.data || []);
    const seenC = new Set();
    const uniqueChengyu = allChengyu.filter(e => seenC.has(e.simplified) ? false : (seenC.add(e.simplified), true));
    const cidx = dayOfYear % uniqueChengyu.length;
    const selectedC = uniqueChengyu[cidx];
    console.log(`[getWordOfDay] chengyu idx=${cidx}/${uniqueChengyu.length} selected=${selectedC.simplified}`);
    const { data: crow } = await supabase.from('entries')
      .select('simplified, traditional, pinyin, definitions, hsk_level, frequency_rank')
      .eq('simplified', selectedC.simplified)
      .eq('is_chengyu', true);
    return { ...(crow?.[0] ?? selectedC), isChengyu: true };
  }

  // Regular words:
  // SQL logic: WHERE hsk_level IS NOT NULL AND NOT is_chengyu AND NOT variant/classical
  //            ORDER BY hsk_level ASC, frequency_rank ASC NULLS LAST
  // Then deduplicate by simplified (first occurrence = best hsk_level + frequency_rank).
  // Select: unique[dayOfYear % unique.length]
  const FILTERS = [
    ['is_chengyu', 'is', true],
    ['hsk_level', 'is', null],
  ...
  console.log(`[getWordOfDay] isChengyu=false dayOfYear=${dayOfYear} totalMatchingRows=${totalCount}`);
  if (!totalCount) return null;

  const pages = await Promise.all(
    Array.from({ length: Math.ceil(totalCount / 1000) }, (_, i) =>
      applyFilters(
        supabase.from('entries').select('simplified, hsk_level, frequency_rank')
      )
        .order('hsk_level', { ascending: true, nullsFirst: false })
        .order('frequency_rank', { ascending: true, nullsFirst: false })
        .range(i * 1000, i * 1000 + 999)
  ...
  console.log(`[getWordOfDay] unique=${unique.length} idx=${idx} selected=${selected.simplified} HSK=${selected.hsk_level} freq=${selected.frequency_rank}`);
