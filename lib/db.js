...
export async function getEntry(simplified) {
  ...
  processed.sort((a, b) => {
    ...
    if (Math.abs(aDefCount - bDefCount) >= 4) return bDefCount - aDefCount;
    const aAvg = (a.entry.definitions || '').split(' | ').filter(Boolean).reduce((s, d) => s + d.split(/\s+/).length, 0) / (aDefCount || 1);
    const bAvg = (b.entry.definitions || '').split(' | ').filter(Boolean).reduce((s, d) => s + d.split(/\s+/).length, 0) / (bDefCount || 1);
    if (Math.abs(aAvg - bAvg) > 2) return bAvg - aAvg;
