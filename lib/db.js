...
export async function getFlashcards(hskLevel, limit = 20) {
  const { data, error } = await supabase
    .rpc('get_random_hsk_words', { p_hsk_level: hskLevel, p_limit: limit * 2 });
  if (error || !data?.length) {
    const { data: fallback } = await supabase
      ...
      .limit(limit * 2);
    return (fallback || []).filter(e => !isVariantEntry(e.definitions)).slice(0, limit);
  }
  return data.filter(e => !isVariantEntry(e.definitions)).slice(0, limit);
}
...
export async function getDueCards(userId, hskLevel, limit = 20) {
  ...
  if (need > 0) {
    ...
    newCards = (candidates || [])
      .filter(e => !excludeSet.has(e.simplified) && !isVariantEntry(e.definitions))
      .slice(0, need)
