...
function sortByRelevance(a, b, q, englishWord) {
  ...
  // More definitions = primary/main reading (e.g. 和 hé has 6 defs, 和 hè has 3)
...
export async function getHskPage(hskLevel, page = 1, limit = 50) {
  ...
  // Def count before frequency_rank: freq data is per-character not per-reading,
  // so rare readings can share a good rank (e.g. 后 "empress" rank=42, "behind" rank=2191).
  const bySimplified = new Map();
  for (const entry of all) {
    ...
    if (!existing) {
      ...
      if (bDefs !== aDefs) {
        ...
        const aLen = (existing.definitions || '').length;
        const bLen = (entry.definitions || '').length;
        if (bLen !== aLen) {
          if (bLen > aLen) bySimplified.set(key, entry);
        } else {
          const aFreq = existing.frequency_rank ?? Infinity;
          const bFreq = entry.frequency_rank ?? Infinity;
          if (bFreq < aFreq) bySimplified.set(key, entry);
        }
      }
    }
  }
...
// entry with the most definitions (most complete = primary reading). Frequency
// rank is used only as final tiebreaker because it is assigned per-character,
// not per-reading, causing rare readings to share a good rank.
  ...
  for (const e of entries) {
    ...
    if (!existing) {
      ...
      if (bDefs !== aDefs) {
        ...
        const aLen = (existing.definitions || '').length;
        const bLen = (e.definitions || '').length;
        if (bLen !== aLen) {
          if (bLen > aLen) map.set(key, e);
        } else {
          const aFreq = existing.frequency_rank ?? Infinity;
          const bFreq = e.frequency_rank ?? Infinity;
          if (bFreq < aFreq) map.set(key, e);
        }
      }
    }
  }
