...
export async function getWordOfDay() {
  ...
  // Pool: WHERE hsk_level IS NOT NULL ORDER BY hsk_level ASC, frequency_rank ASC NULLS LAST
  // Deduplicate by simplified (first occurrence = primary/most common reading).
  // Cycle through with dayOfYear % unique.length so index stays in HSK 1 territory.
  const { count: totalCount } = await supabase.from('entries')
    .select('*', { count: 'exact', head: true })
    .not('hsk_level', 'is', null);

  console.log(`[getWordOfDay] dayOfYear=${dayOfYear} totalCount=${totalCount}`);
