...
function mergePinyinGroup(group) {
  ...
  const BOUND_LITERARY_RE = /^\((literary|archaic|classical)\)/i;
...
export async function getRadicalPage(radical, page = 1, limit = 50) {
  ...
  // best reading wins in the group sort below. Also query by traditional column so
  // characters like 乾 (which has its "dry/gān" reading stored under simplified='干')
  // are found correctly — their primary meaning lives in a different simplified form.
  const charSet = new Set(chars);
  const [simpResult, tradResult] = await Promise.all([
  ...
  if (simpResult.error) throw simpResult.error;
  if (tradResult.error) throw tradResult.error;

  // Group all rows per character (using simplified when it's a radical char, else traditional)
  // and merge pinyin readings with the shared helper so the primary reading always wins.
  const groups = new Map();
  const seen = new Set();
  function addEntry(groupKey, entry) {
    const uid = `${entry.simplified}|${entry.pinyin}|${entry.definitions}`;
    if (seen.has(uid)) return;
    seen.add(uid);
    if (!groups.has(groupKey)) groups.set(groupKey, []);
    groups.get(groupKey).push(entry);
  }
  for (const entry of simpResult.data || []) {
    addEntry(entry.simplified, entry);
  }
  for (const entry of tradResult.data || []) {
    // Group under traditional form when it's one of the radical's chars
    // (e.g. gan1 entries with simplified='干', traditional='乾' → group under '乾')
    const key = charSet.has(entry.traditional) ? entry.traditional : entry.simplified;
    addEntry(key, entry);
  }
