...
export async function getRadicalPage(radical, page = 1, limit = 50) {
  // Fetch all matching rows so we can deduplicate by simplified in JS.
  // Multiple rows per character exist (different readings, surname variants, etc.)
  // and must be collapsed to one card each.
  const { data, error } = await supabase
    .from('entries')
    .select('simplified, traditional, pinyin, definitions, hsk_level, frequency_rank')
    .eq('radical', radical)
  ...
  // Group all rows per simplified, then pick the best representative for each.
  // DB ordering handles HSK/freq ties inconsistently (e.g. all 和 readings share
  // rank 169), so we re-sort each group using definition count as tiebreaker.
  ...
  // Sort the deduplicated entries for consistent card ordering.
  deduped.sort((a, b) => {
    const aHsk = a.hsk_level ?? 999;
    const bHsk = b.hsk_level ?? 999;
    if (aHsk !== bHsk) return aHsk - bHsk;
